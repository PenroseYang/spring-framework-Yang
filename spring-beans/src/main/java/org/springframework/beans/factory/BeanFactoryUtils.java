...
public abstract class BeanFactoryUtils {
	...
	/*
	在Spring中，如果beanName以&开头，那么Spring会尝试返回一个FactoryBean的实例，而不是FactoryBean创建的bean实例。这是Spring的一种特殊处理方式。

	例如，如果你有一个FactoryBean，它的名字是myFactoryBean，那么：

	使用getBean("myFactoryBean")，Spring会返回由myFactoryBean工厂创建的bean实例。
	使用getBean("&myFactoryBean")，Spring会返回myFactoryBean工厂本身的实例。
	这种处理方式主要是为了让用户可以直接获取到FactoryBean的实例，以便进行更多的操作，如调用FactoryBean的其他方法。
	 */
