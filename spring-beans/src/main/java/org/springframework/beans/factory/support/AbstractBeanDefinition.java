 ...
 * <p>
 * <p>
 * 这个类还是比较重要的，Spring中Bean的大部分属性都保存在这个类里面了
 */
...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	 ...
	 * <p>
	 * 1. Singleton ：一个Spring 容器中只有一个Bean 的实例，此为Spring 的默认配置，全容器共享一个实例。(默认实现)
	...
	/**
	 * 这两个是工厂方法了，通过FactoryBean 来生产各种的Bean
	 * BeanFactory是容器
	 */
	@Nullable
	...
	/**
	 * 这是从 <constroctor-args> 标签里面解析出的那些 key-value
	 */
	@Nullable
	 ...
	 * <p>
	 * 这个字段，在BeanDefinition往Map 里面进行注册的时候会用到，如果Map里面已经有了这个Bean，而接下来注册的
	 * Bean比之前的那个role要高，优先级要高的话，就把之前那个覆盖掉。
	 */
