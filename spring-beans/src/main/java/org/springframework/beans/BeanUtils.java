...
public abstract class BeanUtils {
	...
	/**
	 * 使用给定的构造函数实例化一个类的便捷方法。
	 * <p>注意，如果给定的构造函数是不可访问的（即，非公共的），此方法会尝试设置构造函数为可访问的，并支持具有可选参数和默认值的Kotlin类。
	 *
	 * @param ctor 要实例化的构造函数
	 * @param args 要应用的构造函数参数（对于具有可选参数和默认值的Kotlin类，如果需要，使用{@code null}表示未指定的参数）
	 * @return 新的实例
	...
	public static <T> T instantiateClass(Constructor<T> ctor, Object... args) throws BeanInstantiationException {
		...
		try {
			...
			throw new BeanInstantiationException(ctor, "它是一个抽象类吗？", ex);
		} catch (IllegalAccessException ex) {
			throw new BeanInstantiationException(ctor, "构造函数是否可访问？", ex);
		} catch (IllegalArgumentException ex) {
			throw new BeanInstantiationException(ctor, "构造函数的参数非法", ex);
	...
	/**
	 * 返回提供的类的主构造函数。
	 * 对于Kotlin类，这将返回与Kotlin主构造函数对应的Java构造函数
	 * （如Kotlin规范中定义）。否则，特别是对于非Kotlin类，这只返回{@code null}。
	 *
	 * @param clazz 要检查的类
	...
	/**
	 * 查找具有给定方法名和给定参数类型的方法，该方法在给定类或其超类上声明。优先选择公共方法，
	 * 但也可以返回受保护的、包访问的或私有方法。
	 * <p>首先检查 {@code Class.getMethod}，然后回退到
	 * {@code findDeclaredMethod}。这允许在受限制的Java安全设置的环境中
	 * 无问题地查找公共方法。
	 *
	 * @param clazz      要检查的类
	...
	/**
	 * 查找具有给定方法名和给定参数类型的方法，该方法在给定类或其超类上声明。将返回公共、受保护、包访问或私有方法。
	...
	public static PropertyEditor findEditorByConvention(@Nullable Class<?> targetType) {
		...
		try {
			...
			if (logger.isDebugEnabled()) {
				logger.debug("No property editor [" + editorName + "] found for type " + targetTypeName + " according to 'Editor' suffix convention");
			}
			unknownEditorTypes.add(targetType);
			return null;
		}
