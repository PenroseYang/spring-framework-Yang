...
public abstract class AbstractPlatformTransactionManager implements PlatformTransactionManager, Serializable {
	...
	public final TransactionStatus getTransaction(@Nullable TransactionDefinition definition) throws TransactionException {
		/**
		 * (1) 从当前的 TransactionManager 里面找一下有没有对应于当前 Thread的连接
		 */
		...
		/**
		 * (2) 判断当前线程是否存在事务
		 * 如果从事务管理器里面拉出来的，对应于当前线程的事务不为空，当前的线程有事务
		 * 就复用这个事务
		 */
		...
		// 走到这个位置上的话，就说明一定上层没有事务了
		// 如果当前线程不存在事务，但是propagationBehavior却被声明为PROPAGATION MANDATORY抛出异常
	...
	private TransactionStatus handleExistingTransaction(
		...
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_REQUIRES_NEW) {
			...
			try {
				...
				// 这里面基本上是对连接池和连接的操作，生成了一个新的连接出来
				doBegin(transaction, definition);
				// 把当前的事务绑定到事务管理器上面去，设置成当前正在生效的事务
				prepareSynchronization(status, definition);
				return status;
	 ...
	 * <p>
	 * <p>
	 * 回滚处理，如果是 RuntimeException 或者 Error ，catch 住了之后就会走到这里来
	 */
